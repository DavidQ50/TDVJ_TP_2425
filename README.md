...
## Classes:
### Animation.cs:
A classe Animation representa as animações no contexto do jogo. Ela faz a gestão dos frames, do controlo e da transição das animações. A classe tem vários construtores que inicializam as animações com diferentes parâmetros(personagem, estado, posição, cor, rotação, escala e profundidade). A classe atualiza as texturas, regiões de recorte (SourceRectangle) e gere a transição entre frames com base no ritmo (pace) da animação. A cada atualização, ajusta as propriedades visuais e desenha o frame atual na tela. Assim, a classe Animation é usada para criar e gerir as animações no jogo.

### Ball.cs:
A classe Ball representa a lógica das bolas que interagem com o ambiente. Ela contém os atributos principais como velocity (velocidade da bola em X e Y), gravitaion (gravidade aplicada à bola), points e health (pontuação e vida da bola), uniqueColor (cor única da bola) e countHitsWall (contador de colisões com as paredes). Esta classe atualiza a posição da bola com base na sua velocidade, verifica as colisões com as bordas da tela e o chão, invertendo a direção da velocidade quando necessário, e incrementa o contador de colisões. Esta classe trata também as colisões com outros objetos do jogo, ajustando a direção da bola com base na posição da colisão e atualizando os atributos do objeto atingido, como a cor e a vida restante. Em resumo, a classe lida com o comportamento das bolas no jogo, incluindo movimentação e colisões, além de implementar um sistema de física simplificado com eventos de colisão.
